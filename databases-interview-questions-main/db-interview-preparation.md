...
**Reporting and Analytics**: Systems that are skewed towards **read operations** over write operations benefit from denormalization. When designing systems for reporting or analytics, it's common practice to have dedicated read replicas that are denormalized for improve read performance.
...
> **Reading time:** ~5 minutes · **Database:** PostgreSQL

### Mental model: index = book index

PostgreSQL can find rows in two main ways:

| Approach | What happens | When it hurts |
|----------|----------------|---------------|
| **Sequential Scan** | Reads every row in the table | Large tables, frequent `WHERE` / `JOIN` / `ORDER BY` |
| **Index Scan** | Jumps to matching rows via the index | Rare on tiny tables; wrong index design |

**Without an index** on a large table, a filter like `WHERE email = 'a@test.com'` forces PostgreSQL to check **every row** — **O(n)**.

**With a B-Tree index** (PostgreSQL’s default), the planner can traverse a sorted tree structure — roughly **O(log n)** — similar in spirit to binary search.
...
After creating the index, always confirm real behavior with:
...
Look for `Index Scan` or `Bitmap Index Scan` — not only `Seq Scan`.

### How B-Tree indexes work (default)

- Index entries are stored in a **balanced tree**, sorted by indexed column(s).
- Leaf nodes point to **heap tuples** (table row locations) or enough info to fetch them.
- Works well for: `=`, `<`, `>`, `<=`, `>=`, `BETWEEN`, `IN`, and many `ORDER BY` / `JOIN` patterns on indexed columns.

---

### Composite indexes and column order

A **composite index** indexes multiple columns in a fixed **left-to-right** order.

```sql
CREATE INDEX idx_orders_user_status ON orders(user_id, status);
```

Index order: **`user_id` first**, then **`status`**.

Think of it like a phone book sorted by **last name**, then **first name** — you can find “Smith, John” quickly, but not all “John” entries across every last name.

#### Efficient (left-most prefix used)
...
#### Usually inefficient (skips left-most column)

```sql
WHERE status = 'PAID';
```

PostgreSQL may still use the index in some cases (e.g. bitmap scans on selective filters), but **do not rely on it** — design for how queries actually filter.

#### Rule of thumb

> A composite index helps most when `WHERE`, `JOIN`, or `ORDER BY` conditions **start from the left-most indexed column(s)**.
...
Not `(created_at, company_id)` — filtering usually starts with `company_id` (equality), then `created_at` (range).

**Column order tips:**

1. Put **equality** columns first (`=`, `IN`).
2. Put **range** columns after (`>`, `<`, `BETWEEN`).
3. Put **high-selectivity** columns earlier when unsure (narrows the tree faster).

---

### Index trade-offs (why “index everything” fails)

Indexes speed up **reads** but cost on **writes**:

| Benefit | Cost |
|---------|------|
| Faster `SELECT`, many `JOIN`s | Slower `INSERT`, `UPDATE`, `DELETE` |
| Faster lookups & sorts | Extra **disk** and **memory** |
| Better planner choices | **VACUUM** / maintenance on dead tuples + index bloat |

Every write must update **all** relevant indexes on that table. Add indexes for **proven hot queries**, not hypothetical ones.
...
|------|----------|---------|
| **B-Tree** | Default; equality, ranges, sorting | `CREATE INDEX idx ON users(email);` |
| **Hash** | Equality only (`=`); rare in practice | `CREATE INDEX idx ON users USING HASH(email);` |
| **GIN** | JSONB, arrays, full-text search | See below |
| **GiST** | Geometry, full-text, nearest-neighbor | Spatial / advanced search |
| **BRIN** | Very large, naturally ordered data (time series) | Timestamp on append-only logs |
...
Useful for queries like `metadata @> '{"brand": "X"}'` or `metadata ? 'key'`.

#### Partial index (bonus — high impact)

Index only rows you query often:
...
Smaller index, faster writes, targeted reads.

#### Unique index

Enforces uniqueness and speeds lookups:
...
1. **Seq Scan vs Index Scan** — know the difference.
2. **B-Tree** — default; ~O(log n) lookups.
3. **Composite order** — left-most column matters; equality before range.
4. **Too many indexes** — slower writes, more storage.
5. **`EXPLAIN ANALYZE`** — verify the planner uses your index.
6. **GIN for JSONB** — not B-Tree for containment operators.

### One-line summary

> Indexes trade write cost and storage for read speed; in PostgreSQL, design composite indexes to match real `WHERE` column order, default to B-Tree, and prove usage with `EXPLAIN ANALYZE`.
