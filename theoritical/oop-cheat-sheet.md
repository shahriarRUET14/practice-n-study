...
graph TD
    ...
    OOP --> P[Pillars]
    OOP --> R[Relationships]
    OOP --> M[Modeling & Design]

    C --> CA[Attributes / Fields]
    C --> CM[Methods / Behaviors]
    C --> CC[Constructors]
    C --> CAx[Access Modifiers]
    CAx --> Pub[Public]
    CAx --> Pri[Private]
    CAx --> Pro[Protected]
    CAx --> Def[Default]
    ...
    Inh --> Single[Single Inheritance]
    Inh --> Multi[Multilevel / Hierarchical]
    Inh --> Impl[Interface Inheritance]

    Pol --> Compile[Compile-time (Overloading)]
    Pol --> Runtime[Runtime (Overriding)]

    R --> Assoc[Association]
    R --> Agg[Aggregation]
    R --> Comp[Composition]
    R --> Dep[Dependency]
    R --> Real[Realization (Interface)]

    M --> Intf[Interface]
    M --> AbsCls[Abstract Class]
    M --> Pack[Packages / Modules]
    M --> Patterns[Design Patterns]
    Patterns --> Creational[Creational]
    Patterns --> Structural[Structural]
    Patterns --> Behavioral[Behavioral]
```
